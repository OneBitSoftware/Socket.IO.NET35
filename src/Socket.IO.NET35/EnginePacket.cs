...
namespace Socket.IO.NET35
{
    public class EnginePacket
    {
        ...
        private static readonly EnginePacket _err = new EnginePacket(EnginePacket.ERROR, "parser error");

        public string Type { get; set; }
        public object Data { get; set; }


        public EnginePacket(string type, object data)
        {
            this.Type = type;
            this.Data = data;
        }

        public EnginePacket(string type)
        {
            this.Type = type;
            this.Data = null;
        }
        ...
        private static EnginePacket DecodeBase64Packet(string msg)
        {
            int type;
            ...
            return new EnginePacket(_packetsList[(byte)type], decodedFromBase64);
        }

        internal static EnginePacket DecodePacket(byte[] data)
        {
            int type = data[0];
            var byteArray = new byte[data.Length - 1];
            Array.Copy(data, 1, byteArray, 0, byteArray.Length);
            return new EnginePacket(_packetsList[(byte)type], byteArray);
        }



        internal static void EncodePayload(EnginePacket[] packets, IEncodeCallback callback)
        {
            if (packets.Length == 0)
            {
